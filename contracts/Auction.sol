...
contract Auction {
    ...
    function bid(uint8 _price) public validState(State.STARTED) {
        ...
        // ** Start code here.  ** /
        // Các bidder đặt giá thầu
        // Nếu giá đấu thầu nhỏ hơn giá mới nhất (hiện tại) + bước tối thiếu
        if (_price < currentPrice + rule.minimumStep) revert();

        // Nếu giá đấu thầu lớn hơn số token hiện có của bidder + số token đã đặt cọc
        if (_price > currentBidder.token + currentBidder.deposit) revert();

        // Sau khi đấu xong nếu trúng thầu thì lượng token còn lại của bidder
        // sẽ = tổng lượng token hiện tại + lượng token đã đặt cọc - giá đấu thầu
        currentBidder.token =
            currentBidder.token +
            currentBidder.deposit -
            _price;

        // Chuyển token sang đặt cọc (deposit)
    ...
    function getDeposit() public validState(State.CLOSING) {
        ...
        // Số lượng token hiện tại cộng thêm số lượng token đã đặt cọc để thu hồi lại
